...
StreamingWebViewPage::StreamingWebViewPage(QWidget* parent)
    ...
    topBar_->setStyleSheet(
        "QWidget { background-color: #101010; border-bottom: 1px solid #2a2a2a; }"
        "QToolButton { color: #ffffff; background-color: #2a2a2a; border: 1px solid #3a3a3a; border-radius: 10px; padding: 10px 14px; font-weight: 700; }"
        "QToolButton:hover { background-color: #3a3a3a; }"
        "QLabel { color: #e0e0e0; font-size: 16px; font-weight: 700; }"
    );
...
void StreamingWebViewPage::keyPressEvent(QKeyEvent* event) {
    ...
    if (view_) {
        QCoreApplication::sendEvent(view_, event);
