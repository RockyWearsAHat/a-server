...
static bool isProbablyTextFile(const QString &name) {
  ...
  return lower.endsWith(".txt") || lower.endsWith(".md") ||
         ...
         lower.endsWith(".xml") || lower.endsWith(".ini") ||
         lower.endsWith(".cfg") || lower.endsWith(".csv") ||
         lower.endsWith(".cpp") || lower.endsWith(".h") ||
         lower.endsWith(".hpp") || lower.endsWith(".c") ||
         lower.endsWith(".py") || lower.endsWith(".js") ||
         lower.endsWith(".ts") || lower.endsWith(".qml") ||
...
NASPage::NASPage(QWidget *parent) : QWidget(parent) {
  ...
  previewInfo_->setObjectName("nasPreviewInfo");
  ...
  upBtn_->setProperty("variant", "secondary");
  refreshBtn_->setProperty("variant", "secondary");
  mkdirBtn_->setProperty("variant", "secondary");
  renameBtn_->setProperty("variant", "secondary");
  uploadBtn_->setProperty("variant", "secondary");
...
void NASPage::refresh() { requestList(currentPath_); }
...
QNetworkReply *NASPage::postUpload(const QString &dir, const QString &name,
...
void NASPage::requestList(const QString &path) {
  ...
  connect(reply, &QNetworkReply::finished, this, [this, reply]() {
    ...
    for (const auto &v : items) {
      ...
      const QString prefix = e.isDir ? QString::fromUtf8("\xF0\x9F\x93\x81 ")
...
void NASPage::requestTextPreview(const QString &path) {
  ...
  connect(reply, &QNetworkReply::finished, this, [this, reply]() {
    ...
    if (status != 200) {
      textPreview_->setPlainText(
          QString("(text preview failed: HTTP %1)").arg(status));
      return;
    }
...
void NASPage::requestImagePreview(const QString &path) {
  ...
  connect(reply, &QNetworkReply::finished, this, [this, reply]() {
    ...
    if (status != 200) {
      imagePreview_->setText(
          QString("(image preview failed: HTTP %1)").arg(status));
      return;
    }

    QPixmap pm;
    pm.loadFromData(body);
    imagePreview_->setPixmap(pm.scaled(
        imagePreview_->size(), Qt::KeepAspectRatio, Qt::SmoothTransformation));
  });
}
...
void NASPage::mkdirInCurrent() {
  ...
  connect(reply, &QNetworkReply::finished, this, [this, reply]() {
    ...
    if (status != 201) {
      statusLabel_->setText(
          QString("Failed to create folder (HTTP %1)").arg(status));
      return;
    }
...
void NASPage::renameSelected() {
  ...
  const QString newName = QInputDialog::getText(
      this, "Rename", "New name:", QLineEdit::Normal, e.name, &ok);
  if (!ok)
    return;
  const QString trimmed = newName.trimmed();
  if (trimmed.isEmpty() || trimmed == e.name)
    return;
...
void NASPage::deleteSelected() {
  ...
  const auto res = QMessageBox::question(
      this, "Delete",
      QString("Delete '%1'? This cannot be undone.").arg(e.name));
