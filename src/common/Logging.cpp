...
static QString isoTimeFromMs(uint64_t msSinceEpoch) {
    const QDateTime dt = QDateTime::fromMSecsSinceEpoch(static_cast<qint64>(msSinceEpoch));
...
static void writeLineToSinks(const std::string& line, LogLevel level) {
    ...
    if (g_mirrorEnabled) {
        std::ostream& out = (level >= LogLevel::Error) ? *g_mirrorCerr : *g_mirrorCout;
        out << line << std::endl;
    }
}
...
public:
    LoggerLineStreamBuf(std::streambuf* fallback, LogLevel level, std::string category)
...
private:
    void flushLine() {
        ...
        // Prevent deadlocks/recursion if a write happens while the logger itself is logging.
        if (g_inRedirectWrite) {
            if (m_fallback) {
                m_fallback->sputn(m_line.data(), static_cast<std::streamsize>(m_line.size()));
                m_fallback->sputc('\n');
            }
            m_line.clear();
            return;
...
void InitAppLogging(const QString& logFilePath) {
    ...
    if (!logFilePath.isEmpty()) {
        ...
        if (fi.dir().exists() == false && !fi.path().isEmpty() && fi.path() != ".") {
            fi.dir().mkpath(".");
        }
    }
    ...
    Logger::Instance().SetLevel(parseLevel(qEnvironmentVariable("AIO_LOG_LEVEL")));
    ...
    g_coutBuf = std::make_unique<LoggerLineStreamBuf>(g_origCoutBuf, LogLevel::Info, "STDOUT");
    g_cerrBuf = std::make_unique<LoggerLineStreamBuf>(g_origCerrBuf, LogLevel::Error, "STDERR");
    std::cout.rdbuf(g_coutBuf.get());
    std::cerr.rdbuf(g_cerrBuf.get());

    Logger::Instance().Log(LogLevel::Info, "main", "Logging initialized: " + logFilePath.toStdString());
}
