...
ROMMetadata ROMMetadataAnalyzer::Analyze(const std::vector<uint8_t> &romData) {
  ...
  if (romData.size() >= 0xB0) {
    metadata.gameCode =
        std::string(reinterpret_cast<const char *>(&romData[0xAC]), 4);
  ...
  case Region::NorthAmerica:
    std::cout << "North America (E)";
    break;
  case Region::PAL:
    std::cout << "PAL/Europe (P)";
    break;
  case Region::Korea:
    std::cout << "Korea (K)";
    break;
...
Region
ROMMetadataAnalyzer::DetectRegionFromGameCode(const std::string &gameCode) {
  ...
  switch (regionChar) {
  case 'E':
    return Region::NorthAmerica; // Europe/US in GBA context (Nintendo used E
                                 // for US)
  case 'P':
    return Region::PAL; // PAL (Europe, Australia)
  case 'J':
    return Region::Japan; // Japan
  case 'K':
    return Region::Korea; // Korea
  default:
    return Region::Unknown;
  }
}
...
Language ROMMetadataAnalyzer::InferLanguageFromRegion(Region region) {
  ...
  case Region::PAL:
    return Language::English; // Most PAL games primary lang is English
  case Region::Korea:
...
std::string
ROMMetadataAnalyzer::ExtractGameTitle(const std::vector<uint8_t> &romData) {
...
SaveType
ROMMetadataAnalyzer::DetectSaveType(const std::vector<uint8_t> &romData) {
  ...
  if (Contains(romData, "EEPROM_V")) {
    std::cout << "[ROMAnalyzer] Found EEPROM_V marker (64K default)"
              << std::endl;
  ...
  if (Contains(romData, "FLASH_V")) {
    std::cout << "[ROMAnalyzer] Found FLASH_V marker (512K default)"
              << std::endl;
    return SaveType::Flash512;
  }
...
SaveType
ROMMetadataAnalyzer::AnalyzeSaveBehavior(const std::vector<uint8_t> &romData) {
  ...
  // The driver code strings are inserted into ROM by devkit and are reliable
  // markers Only fall back to pattern analysis if string detection fails

  // For now, return Auto to let string detection handle it
  return SaveType::Auto;
}

bool ROMMetadataAnalyzer::Contains(const std::vector<uint8_t> &romData,
                                   const std::string &pattern) {
  if (pattern.length() > romData.size())
    return false;
  auto it = std::search(romData.begin(), romData.end(), pattern.begin(),
                        pattern.end());
  return it != romData.end();
}

bool ROMMetadataAnalyzer::ContainsRegex(const std::vector<uint8_t> &romData,
