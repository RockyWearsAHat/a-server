...
namespace AIO::Emulator::GBA {
    ...
    bool GBA::LoadROM(const std::string& path) {
        ...
        if (!file.is_open()) {
            std::cout << "[LoadROM] File not found at: " << path << ", trying alternate paths" << std::endl;
        ...
        if (file.read(reinterpret_cast<char*>(buffer.data()), size)) {
            ...
            std::cerr << "[LoadROM] ===== Metadata Analysis Complete =====" << std::endl;
            std::cerr.flush();
            ...
            std::cout << "[LoadROM] Attempting to open save file, is_open=" << saveFile.is_open() << std::endl;
            if (saveFile.is_open()) {
                std::streamsize saveSize = saveFile.tellg();
                std::cout << "[LoadROM] Save file found: " << savePath << " (" << saveSize << " bytes)" << std::endl;
                saveFile.seekg(0, std::ios::beg);
                
                // Verify seek worked
                std::streampos pos = saveFile.tellg();
                std::cout << "[LoadROM] After seekg(0), position is: " << pos << std::endl;
                
                std::vector<uint8_t> saveData(saveSize);
                std::cout << "[LoadROM] Created saveData vector of size: " << saveData.size() << std::endl;
                
                if (saveFile.read(reinterpret_cast<char*>(saveData.data()), saveSize)) {
                    std::cout << "[LoadROM] Successfully read " << saveSize << " bytes from save file" << std::endl;
                    ...
                    for (size_t i = 0x10; i < std::min((size_t)saveSize, size_t(0x18)); i++) {
                        std::cout << std::setw(2) << std::setfill('0') << (int)saveData[i] << " ";
                    }
                    std::cout << std::dec << std::endl;
                    std::cout << "[LoadROM] Bytes at offset 0x18-0x2f: " << std::hex;
                    for (size_t i = 0x18; i < std::min((size_t)saveSize, size_t(0x30)); i++) {
                        std::cout << std::setw(2) << std::setfill('0') << (int)saveData[i] << " ";
                    }
                    std::cout << std::dec << std::endl;
                    
                    // DEBUG: Verify saveData immediately before passing to LoadSave
                    std::cout << "[LoadROM DEBUG] About to call LoadSave. saveData[0x10-0x27]: " << std::hex;
                    for (size_t i = 0x10; i < std::min((size_t)saveSize, size_t(0x28)); i++) {
                        std::cout << std::setw(2) << std::setfill('0') << (int)saveData[i] << " ";
                    }
                    std::cout << std::dec << std::endl;
                    ...
                    std::cout << "[LoadROM] Verification: GetSaveData() returns " << loadedData.size() << " bytes" << std::endl;
            ...
            std::cout << "[LoadROM] CPU Reset complete. PC=0x" << std::hex << cpu->GetRegister(15) 
                      << " CPSR=0x" << cpu->GetCPSR() << std::dec << std::endl;
            std::cout << "[LoadROM] IME=" << memory->Read16(0x04000208) 
                      << " IE=0x" << std::hex << memory->Read16(0x04000200) 
                      << " IF=0x" << memory->Read16(0x04000202) << std::dec << std::endl;
            return true;
        }
    ...
    void GBA::ConfigureBootStateFromMetadata(const ROMMetadata& metadata) {
        std::cout << "[ConfigureBoot] Configuring boot state based on detected metadata..." << std::endl;
        ...
        switch (metadata.language) {
            ...
            case Language::Korean: std::cout << "Korean"; break;
            default: std::cout << "Unknown"; break;
        }
        std::cout << std::endl;

        // Apply game-specific boot configurations based on metadata
        // No hardcoded patches - everything is derived from the ROM's actual structure
    ...
    int GBA::Step() {
        ...
        if (prevPc == lastPC) {
            ...
            if (pcRepeatCount == 10000 && TraceGbaSpam()) {
                ...
                std::cout << "  R0=0x" << std::hex << cpu->GetRegister(0)
                          << " R1=0x" << cpu->GetRegister(1)
                          << " R2=0x" << cpu->GetRegister(2)
                          << " R3=0x" << cpu->GetRegister(3) << std::endl;
                std::cout << "  SP=0x" << cpu->GetRegister(13)
                          << " LR=0x" << cpu->GetRegister(14)
                          << " CPSR=0x" << cpu->GetCPSR() << std::dec << std::endl;
            }
        } else {
            lastPC = prevPc;
        ...
        if (currPc == prevPc) {
            ...
            if (!stallCrashTriggered && stallCycleAccumulator >= STALL_CYCLE_THRESHOLD) {
                Common::Logger::Instance().LogFmt(Common::LogLevel::Fatal, "GBA",
                    "PC stall detected: PC=0x%08X Thumb=%d Cycles=%llu",
                    currPc, IsThumbMode() ? 1 : 0,
    ...
    void GBA::AddBreakpoint(uint32_t addr) {
        if (cpu) cpu->AddBreakpoint(addr);
    }

    void GBA::ClearBreakpoints() {
        if (cpu) cpu->ClearBreakpoints();
    }

    void GBA::SetSingleStep(bool enabled) {
        if (cpu) cpu->SetSingleStep(enabled);
    }
