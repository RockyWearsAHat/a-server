...
bool R3000A::TryHLETrap() {
  ...
  bool handled = PS1HleBios::Dispatch(*ps1, tableId, funcNum);

  if (handled) {
    // HLE handled it and expects normal return to caller ($ra)
    pc = regs[31]; // $ra
    nextPc = pc + 4;
  }
  // If !handled, Dispatch already set PC (table redirect or
  // ReturnFromException)
  return true;
}
