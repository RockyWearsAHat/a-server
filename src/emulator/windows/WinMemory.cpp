...
void WinMemory::Free(uint64_t base) { regions_.erase(base); }
...
uint8_t WinMemory::Read8(uint64_t a) const {
  uint8_t v = 0;
  Read(a, &v, 1);
  return v;
}
uint16_t WinMemory::Read16(uint64_t a) const {
  uint16_t v = 0;
  Read(a, &v, 2);
  return v;
}
uint32_t WinMemory::Read32(uint64_t a) const {
  uint32_t v = 0;
  Read(a, &v, 4);
  return v;
}
uint64_t WinMemory::Read64(uint64_t a) const {
  uint64_t v = 0;
  Read(a, &v, 8);
  return v;
}
...
uint64_t WinMemory::DoParsePE(const std::vector<uint8_t> &data,
                              const std::string &name, uint64_t preferredBase) {
  if (data.size() < 0x40) {
    std::cerr << "[PE] Too small\n";
    return 0;
  }
  ...
  if (e_magic != kMZMagic) {
    std::cerr << "[PE] Bad MZ magic\n";
    return 0;
  }

  const uint32_t e_lfanew =
      *reinterpret_cast<const uint32_t *>(data.data() + 0x3C);
  if (e_lfanew + 0x50 > data.size()) {
    std::cerr << "[PE] e_lfanew OOB\n";
    return 0;
  }

  // PE signature
  const uint32_t peSig =
      *reinterpret_cast<const uint32_t *>(data.data() + e_lfanew);
  if (peSig != kPESignature) {
    std::cerr << "[PE] Bad PE signature\n";
    return 0;
  }
  ...
  const uint32_t sectionAlign =
      *reinterpret_cast<const uint32_t *>(ohdr + 0x20);
  ...
  if (numDataDirs > (uint32_t)kDirReloc) {
    ...
    relocSize =
        *reinterpret_cast<const uint32_t *>(dataDirs + kDirReloc * 8 + 4);
  }

  // Choose load address
  uint64_t loadBase = preferredBase
                          ? preferredBase
  ...
  if (!MapSections(data, loadBase, numSections, sectionStart))
    return 0;
  ...
  // Resolve imports
  if (importRVA) {
    if (!ResolveImports(data, loadBase, importRVA))
      return 0;
  }
...
bool WinMemory::MapSections(const std::vector<uint8_t> &data, uint64_t loadBase,
  ...
  for (uint32_t i = 0; i < numSections; ++i) {
    ...
    const uint32_t virtualSize =
        *reinterpret_cast<const uint32_t *>(data.data() + off + 8);
    const uint32_t virtualAddr =
        *reinterpret_cast<const uint32_t *>(data.data() + off + 12);
    const uint32_t rawSize =
        *reinterpret_cast<const uint32_t *>(data.data() + off + 16);
    const uint32_t rawOffset =
        *reinterpret_cast<const uint32_t *>(data.data() + off + 20);
...
bool WinMemory::ApplyRelocations(const std::vector<uint8_t> &data,
                                 uint64_t loadBase, uint64_t origBase,
                                 uint64_t relocRVA, uint32_t relocSize) {
  const int64_t delta =
      static_cast<int64_t>(loadBase) - static_cast<int64_t>(origBase);
  if (delta == 0)
    return true;
  ...
  while (off < end) {
    ...
    const uint32_t pageRVA =
        *reinterpret_cast<const uint32_t *>(data.data() + off);
    const uint32_t blockSize =
        *reinterpret_cast<const uint32_t *>(data.data() + off + 4);
    if (blockSize < 8)
      break;

    const uint32_t numEntries = (blockSize - 8) / 2;
    for (uint32_t i = 0; i < numEntries; ++i) {
      if (off + 8 + i * 2 + 2 > data.size())
        break;
      const uint16_t entry =
          *reinterpret_cast<const uint16_t *>(data.data() + off + 8 + i * 2);
...
bool WinMemory::ResolveImports(const std::vector<uint8_t> &data,
                               uint64_t loadBase, uint64_t importRVA) {
  // IMAGE_IMPORT_DESCRIPTOR: 20 bytes each, ends with all-zero entry
  uint64_t off = importRVA;
  for (;;) {
    if (off + 20 > data.size())
      break;
    const uint32_t origThunkRVA =
        *reinterpret_cast<const uint32_t *>(data.data() + off + 0);
    const uint32_t nameRVA =
        *reinterpret_cast<const uint32_t *>(data.data() + off + 12);
    const uint32_t iatRVA =
        *reinterpret_cast<const uint32_t *>(data.data() + off + 16);
    ...
    if (nameRVA < data.size()) {
      ...
      while (p < reinterpret_cast<const char *>(data.data() + data.size()) &&
             *p)
        dllName +=
    ...
    for (uint32_t slot = 0;; ++slot) {
      ...
      if (thunkOff + 8 > data.size())
        break;

      const uint64_t thunk =
          *reinterpret_cast<const uint64_t *>(data.data() + thunkOff);
      if (thunk == 0)
        break;
      ...
      if (thunk & (1ULL << 63)) {
        ...
        if (ibnOff + 2 < data.size()) {
          const char *p =
              reinterpret_cast<const char *>(data.data() + ibnOff + 2);
          while (
              p < reinterpret_cast<const char *>(data.data() + data.size()) &&
              *p)
            funcName += *p++;
        }
      }
      ...
      if (resolvedAddr == 0) {
        ...
        std::cerr << "[PE] Unresolved import: " << dllName << "!" << funcName
                  << "\n";
      }
