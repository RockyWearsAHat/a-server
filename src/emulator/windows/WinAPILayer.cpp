...
// Well-known fake HANDLE values (must be non-zero, odd numbers are recognisable)
...
WinAPILayer::WinAPILayer(WinMemory& mem)
    : mem_(mem) {
    // Reserve handle slot 0 as invalid
    handles.push_back({HT_INVALID, 0, false});
}
...
uint64_t WinAPILayer::AllocHandle(HandleType type, uint64_t data) {
    ...
    for (size_t i = 1; i < handles.size(); ++i) {
        if (!handles[i].valid) {
            handles[i] = {type, data, true};
            return static_cast<uint64_t>(i) * 4; // HANDLE values are multiples of 4
        }
    }
    handles.push_back({type, data, true});
    return static_cast<uint64_t>(handles.size() - 1) * 4;
}

bool WinAPILayer::FreeHandle(uint64_t h) {
    const size_t idx = static_cast<size_t>(h / 4);
    if (idx >= handles.size() || !handles[idx].valid) return false;
    handles[idx].valid = false;
    return true;
}

WinAPILayer::HandleEntry* WinAPILayer::GetHandle(uint64_t h) {
    const size_t idx = static_cast<size_t>(h / 4);
    if (idx >= handles.size() || !handles[idx].valid) return nullptr;
    return &handles[idx];
}
...
FILE* WinAPILayer::GetFile(uint64_t h) {
    ...
    if (!entry || entry->type != HT_FILE) return nullptr;
    const size_t slot = static_cast<size_t>(entry->data);
    if (slot >= fileSlots_.size()) return nullptr;
...
uint64_t WinAPILayer::Arg(int n) const {
    // n is 1-based
    switch (n) {
        case 1: return cpu_->GetGPR(1);  // RCX
        case 2: return cpu_->GetGPR(2);  // RDX
        case 3: return cpu_->GetGPR(8);  // R8
        case 4: return cpu_->GetGPR(9);  // R9
        default: {
...
uint64_t WinAPILayer::AddStub(const std::string& dll,
                               const std::string& func,
                               APIFunc fn) {
    const std::string key = dll + "!" + func;
    auto it = nameToStub_.find(key);
    if (it != nameToStub_.end()) return it->second;
...
uint64_t WinAPILayer::Resolve(const std::string& dll, const std::string& func) {
    ...
    if (sep != std::string::npos) dllKey = dllKey.substr(sep + 1);
    for (auto& c : dllKey)
...
bool WinAPILayer::Dispatch(uint64_t stubAddr) {
    ...
    if (it == stubDispatch_.end()) return false;
    it->second();
    return true;
}
...
void WinAPILayer::Initialize() {
    ...
    exitStubAddr_ = AddStub("kernel32.dll", "ExitProcess",
                             [this]{ I_ExitProcess(); });
...
#define ALIAS(dll, fn, impl) AddStub(dll, #fn, [this]{ I_##impl(); })
    ...
    ALIAS("kernel32.dll", InitializeCriticalSectionAndSpinCount, InitializeCriticalSection);
    ...
    ALIAS("ntdll.dll", RtlInitializeCriticalSectionAndSpinCount, RtlInitializeCriticalSectionEx);
    ...
    for (const char* lib : {"msvcrt.dll", "ucrtbase.dll",
                             "vcruntime140.dll", "vcruntime140d.dll",
                             "msvcr120.dll", "msvcr110.dll",
                             "ucrtbased.dll", "api-ms-win-crt-heap-l1-1-0.dll",
        ...
        std::string l = lib;
        REG(l, malloc); REG(l, free); REG(l, realloc); REG(l, calloc);
        REG(l, printf); REG(l, fprintf); REG(l, sprintf); REG(l, snprintf);
        REG(l, sscanf); REG(l, vprintf); REG(l, vsprintf); REG(l, vsnprintf);
        REG(l, strlen); REG(l, wcslen);
        REG(l, strcpy); REG(l, strncpy); REG(l, strcat); REG(l, strncat);
        REG(l, strcmp); REG(l, strncmp); REG(l, strdup); REG(l, strchr); REG(l, strstr);
        REG(l, strtod); REG(l, strtol); REG(l, strtoul);
        REG(l, memcpy); REG(l, memmove); REG(l, memset); REG(l, memcmp); REG(l, memchr);
        REG(l, atoi); REG(l, atol); REG(l, atof); REG(l, itoa);
        REG(l, exit); REG(l, abort); REG(l, _exit);
        REG(l, rand); REG(l, srand); REG(l, time); REG(l, clock); REG(l, difftime);
        REG(l, fopen); REG(l, fclose); REG(l, fread); REG(l, fwrite);
        REG(l, fseek); REG(l, ftell); REG(l, feof); REG(l, fflush);
        REG(l, fgets); REG(l, fputs); REG(l, fputc); REG(l, fgetc);
        REG(l, getc); REG(l, putc); REG(l, puts); REG(l, gets);
        REG(l, clearerr); REG(l, ferror); REG(l, rewind); REG(l, _fileno);
        REG(l, _CRT_INIT); REG(l, __CxxFrameHandler3);
        REG(l, __C_specific_handler); REG(l, __chkstk);
        REG(l, __security_cookie); REG(l, __security_check_cookie);
        REG(l, _get_initial_narrow_environment);
        REG(l, _configure_narrow_argv);
        REG(l, _initialize_narrow_environment);
        REG(l, _cexit); REG(l, _conexit);
        REG(l, _initterm); REG(l, _initterm_e);
        REG(l, _register_onexit_function);
        REG(l, _execute_onexit_table);
        REG(l, _crt_atexit); REG(l, atexit);
        REG(l, fesetenv); REG(l, fegetenv);
        REG(l, _controlfp_s); REG(l, _set_app_type);
    }
    ...
    // ── XInput ───────────────────────────────────────────────────────────
    for (const char* lib : {"xinput1_4.dll","xinput1_3.dll","xinput9_1_0.dll"}) {
        std::string l = lib;
...
void WinAPILayer::I_VirtualAlloc() {
    ...
    if (prot & 0x10 || prot & 0x20 || prot & 0x40) mprot = WinMemory::PAGE_EXECUTE_READWRITE;
    Ret(mem_.Allocate(base, size, mprot));
}
void WinAPILayer::I_VirtualFree()    { mem_.Free(Arg(1)); Ret(1); }
void WinAPILayer::I_VirtualProtect() {
    ...
    if (pOld) mem_.Write32(pOld, WinMemory::PAGE_READWRITE);
    Ret(1);
}
...
void WinAPILayer::I_CreateFileA() {
    ...
    if (create == 1 /*CREATE_NEW*/ || create == 2 /*CREATE_ALWAYS*/) mode = "wb+";
    FILE* f = std::fopen(path.c_str(), mode);
    if (!f) { lastError = 2; Ret(static_cast<uint64_t>(-1)); return; } // INVALID_HANDLE
    size_t slot = fileSlots_.size();
    for (size_t i = 0; i < fileSlots_.size(); ++i) {
        if (!fileSlots_[i]) { slot = i; fileSlots_[i] = f; goto found; }
    }
    fileSlots_.push_back(f);
found:
    Ret(AllocHandle(HT_FILE, static_cast<uint64_t>(slot)));
}
void WinAPILayer::I_CreateFileW() { I_CreateFileA(); } // simplified: name already copied
...
void WinAPILayer::I_WriteFile() {
    ...
    if (!buf || !nReq) { Ret(1); return; }
    std::vector<uint8_t> tmp(nReq);
    mem_.Read(buf, tmp.data(), nReq);
    size_t written = 1;
    if (f) written = std::fwrite(tmp.data(), 1, nReq, f);
    if (pWrit) mem_.Write32(pWrit, static_cast<uint32_t>(written));
    Ret(1);
}
...
void WinAPILayer::I_GetSystemInfo() {
    ...
    if (!pSI) return;
    mem_.Write32(pSI + 0,  9);     // wProcessorArchitecture = PROCESSOR_ARCHITECTURE_AMD64
...
void WinAPILayer::I_SetLastError()  { lastError = static_cast<uint32_t>(Arg(1)); Ret(0); }
...
void WinAPILayer::I_QueryPerformanceCounter() {
    ...
    if (pQPC) mem_.Write64(pQPC, GetHostNanos());
    Ret(1);
}
void WinAPILayer::I_QueryPerformanceFrequency() {
    const uint64_t pQPF = Arg(1);
    if (pQPF) mem_.Write64(pQPF, kPerfFreq);
    Ret(1);
}
...
void WinAPILayer::I_HeapFree()    { HeapFree_(Arg(3)); Ret(1); }
void WinAPILayer::I_HeapCreate()  {
    if (heapBase_ == 0) HeapAlloc_(0); // initialize
...
void WinAPILayer::I_LocalFree()   { HeapFree_(Arg(1)); Ret(0); }
void WinAPILayer::I_LocalReAlloc(){ Ret(HeapAlloc_(Arg(2))); }
void WinAPILayer::I_GlobalAlloc() { Ret(HeapAlloc_(Arg(2))); }
void WinAPILayer::I_GlobalFree()  { HeapFree_(Arg(1)); Ret(0); }
void WinAPILayer::I_GetProcessHeap() { Ret(processHeapHandle_); }

void WinAPILayer::I_GetCurrentProcessId() { Ret(static_cast<uint32_t>(getpid())); }
...
void WinAPILayer::I_GetEnvironmentVariableA() {
    ...
    if (!val) { lastError = 203; Ret(0); return; }
    if (buf && size > 0) mem_.WriteStringA(buf, val);
...
void WinAPILayer::I_GetStdHandle() {
    ...
    if (which == static_cast<uint32_t>(-10)) Ret(kHandleStdIn);
    else if (which == static_cast<uint32_t>(-11)) Ret(kHandleStdOut);
    else Ret(kHandleStdErr);
}
...
void WinAPILayer::I_SetFilePointer() {
    ...
    if (f) std::fseek(f, dist, method == 0 ? SEEK_SET : method == 1 ? SEEK_CUR : SEEK_END);
...
void WinAPILayer::I_GetFileSize() {
    FILE* f = GetFile(Arg(1));
    if (!f) { Ret(static_cast<uint64_t>(-1)); return; }
...
void WinAPILayer::I_GetFileSizeEx() {
    ...
    const long cur = std::ftell(f);
    std::fseek(f, 0, SEEK_END);
    if (pSize) mem_.Write64(pSize, static_cast<uint64_t>(std::ftell(f)));
    std::fseek(f, cur, SEEK_SET);
    Ret(1);
}
...
void WinAPILayer::I_FindFirstFileA()     { Ret(static_cast<uint64_t>(-1)); } // INVALID_HANDLE
void WinAPILayer::I_FindFirstFileW()     { Ret(static_cast<uint64_t>(-1)); }
void WinAPILayer::I_FindNextFileA()      { lastError = 18; Ret(0); }
void WinAPILayer::I_FindNextFileW()      { lastError = 18; Ret(0); }
void WinAPILayer::I_FindClose()          { Ret(1); }

void WinAPILayer::I_GetFullPathNameA() {
    const std::string path = mem_.ReadStringA(Arg(1));
    const uint64_t buf = Arg(3);
    if (buf) mem_.WriteStringA(buf, path);
...
void WinAPILayer::I_GetTempPathA() {
    const uint64_t buf = Arg(2);
    if (buf) mem_.WriteStringA(buf, "/tmp/");
    Ret(5);
}
void WinAPILayer::I_GetTempPathW() { I_GetTempPathA(); }

void WinAPILayer::I_GetModuleFileNameA() {
    const uint64_t buf = Arg(2); const uint32_t sz = static_cast<uint32_t>(Arg(3));
    const auto& mods = mem_.GetModules();
    const std::string path = mods.empty() ? "" : mods.front().path;
    if (buf && sz) mem_.WriteStringA(buf, path.substr(0, sz-1));
    Ret(static_cast<uint32_t>(path.size()));
}
void WinAPILayer::I_GetModuleFileNameW() { I_GetModuleFileNameA(); }
void WinAPILayer::I_GetSystemDirectoryA() { const uint64_t b=Arg(1); if(b) mem_.WriteStringA(b,"C:\\Windows\\System32"); Ret(20); }
void WinAPILayer::I_GetSystemDirectoryW() { I_GetSystemDirectoryA(); }
void WinAPILayer::I_GetWindowsDirectoryA(){ const uint64_t b=Arg(1); if(b) mem_.WriteStringA(b,"C:\\Windows"); Ret(10); }
...
void WinAPILayer::I_ExpandEnvironmentStringsA() {
    ...
    if (dst) mem_.WriteStringA(dst, src);
...
void WinAPILayer::I_WideCharToMultiByte() {
    ...
    if (src && dst && dstLen > 0) {
        ...
        while (i < dstLen - 1) {
            ...
            if (!ch) break;
            mem_.Write8(dst + i, static_cast<uint8_t>(ch & 0xFF)); ++i;
        }
        mem_.Write8(dst + i, 0);
        Ret(i);
        return;
...
void WinAPILayer::I_FormatMessageA() { const uint64_t b=Arg(5); if(b) mem_.WriteStringA(b,""); Ret(0); }
void WinAPILayer::I_FormatMessageW() { I_FormatMessageA(); }

// Interlocked
void WinAPILayer::I_InterlockedIncrement() {
    uint64_t a = Arg(1); uint32_t v = mem_.Read32(a) + 1; mem_.Write32(a, v); Ret(v);
}
void WinAPILayer::I_InterlockedDecrement() {
    uint64_t a = Arg(1); uint32_t v = mem_.Read32(a) - 1; mem_.Write32(a, v); Ret(v);
}
void WinAPILayer::I_InterlockedExchange() {
    uint64_t a = Arg(1); uint32_t newV = static_cast<uint32_t>(Arg(2));
    uint32_t old = mem_.Read32(a); mem_.Write32(a, newV); Ret(old);
}
void WinAPILayer::I_InterlockedCompareExchange() {
    uint64_t a = Arg(1); uint32_t exch = static_cast<uint32_t>(Arg(2));
    uint32_t cmp = static_cast<uint32_t>(Arg(3)); uint32_t cur = mem_.Read32(a);
    if (cur == cmp) mem_.Write32(a, exch); Ret(cur);
}
void WinAPILayer::I_InterlockedAdd() {
    uint64_t a = Arg(1); uint32_t addend = static_cast<uint32_t>(Arg(2));
    uint32_t r = mem_.Read32(a) + addend; mem_.Write32(a, r); Ret(r);
}

void WinAPILayer::I_SetUnhandledExceptionFilter() { Ret(0); }
void WinAPILayer::I_UnhandledExceptionFilter()    { Ret(1); }
void WinAPILayer::I_RaiseException()              { cpu_->SetRIP(0xDEAD'BEEF'DEAD'BEEFull); Ret(0); }
void WinAPILayer::I_TerminateProcess()            { I_ExitProcess(); }
void WinAPILayer::I_IsWow64Process()              { const uint64_t p=Arg(2); if(p) mem_.Write32(p,0); Ret(1); }
...
void WinAPILayer::I_MapViewOfFile()      { Ret(mem_.Allocate(0, Arg(5) ? Arg(5) : 0x10000, WinMemory::PAGE_READWRITE)); }
...
void WinAPILayer::I_NtAllocateVirtualMemory() {
    ...
    if (pBase) mem_.Write64(pBase, base);
    if (pSize) mem_.Write64(pSize, size);
    Ret(kSTATUS_SUCCESS);
}
void WinAPILayer::I_NtFreeVirtualMemory()    { mem_.Free(Arg(2)); Ret(kSTATUS_SUCCESS); }
void WinAPILayer::I_NtProtectVirtualMemory() { Ret(kSTATUS_SUCCESS); }
void WinAPILayer::I_NtCreateFile()           { Ret(0xC0000034ull); } // STATUS_OBJECT_NAME_NOT_FOUND
void WinAPILayer::I_NtReadFile()             { Ret(kSTATUS_SUCCESS); }
void WinAPILayer::I_NtWriteFile()            { Ret(kSTATUS_SUCCESS); }
void WinAPILayer::I_NtClose()               { FreeHandle(Arg(1)); Ret(kSTATUS_SUCCESS); }
...
void WinAPILayer::I_RtlFreeHeap()        { HeapFree_(Arg(3)); Ret(1); }
void WinAPILayer::I_RtlReAllocateHeap() {
    const uint64_t oldAddr = Arg(3);
    const uint64_t newSize = Arg(4);
    uint64_t newAddr = HeapAlloc_(newSize);
    if (newAddr && oldAddr) {
        for (uint64_t i = 0; i < newSize; ++i) mem_.Write8(newAddr + i, mem_.Read8(oldAddr + i));
        HeapFree_(oldAddr);
    }
    Ret(newAddr);
}
...
void WinAPILayer::I_RtlGetVersion() {
    ...
    if (!pOS) { Ret(kSTATUS_SUCCESS); return; }
...
void WinAPILayer::I_free()   { HeapFree_(Arg(1)); Ret(0); }
void WinAPILayer::I_realloc() {
    uint64_t old = Arg(1); uint64_t sz = Arg(2);
    uint64_t n = HeapAlloc_(sz);
    if (n && old) { for (uint64_t i = 0; i < sz; ++i) mem_.Write8(n+i, mem_.Read8(old+i)); HeapFree_(old); }
    Ret(n);
}
void WinAPILayer::I_calloc() {
    uint64_t n = Arg(1) * Arg(2);
    uint64_t a = HeapAlloc_(n);
    if (a) for (uint64_t i = 0; i < n; ++i) mem_.Write8(a+i, 0);
    Ret(a);
}
void WinAPILayer::I_printf()   { Ret(0); /* no-op — games rarely need this to print */ }
...
void WinAPILayer::I_wcslen() {
    uint64_t a = Arg(1); uint64_t len = 0;
    while (mem_.Read16(a + len * 2)) ++len;
    Ret(len);
}
void WinAPILayer::I_strcpy()  {
    const std::string s = mem_.ReadStringA(Arg(2));
    mem_.WriteStringA(Arg(1), s); Ret(Arg(1));
}
void WinAPILayer::I_strncpy() {
    const std::string s = mem_.ReadStringA(Arg(2));
    uint64_t n = Arg(3);
    std::string trunc = s.substr(0, n);
    mem_.WriteStringA(Arg(1), trunc); Ret(Arg(1));
}
...
void WinAPILayer::I_strcat() {
    ...
    mem_.WriteStringA(Arg(1), a + b); Ret(Arg(1));
}
...
void WinAPILayer::I_strtol() { Ret(static_cast<int32_t>(std::strtol(mem_.ReadStringA(Arg(1)).c_str(), nullptr, static_cast<int>(Arg(3))))); }
void WinAPILayer::I_strtoul() { Ret(std::strtoul(mem_.ReadStringA(Arg(1)).c_str(), nullptr, static_cast<int>(Arg(3)))); }
void WinAPILayer::I_memcpy() {
    uint64_t d = Arg(1), s = Arg(2);
    uint64_t n = Arg(3);
    for (uint64_t i = 0; i < n; ++i) mem_.Write8(d+i, mem_.Read8(s+i));
    Ret(d);
}
void WinAPILayer::I_memmove() { I_memcpy(); }
void WinAPILayer::I_memset() {
    uint64_t d = Arg(1); uint8_t c = static_cast<uint8_t>(Arg(2)); uint64_t n = Arg(3);
    for (uint64_t i = 0; i < n; ++i) mem_.Write8(d+i, c);
    Ret(d);
}
void WinAPILayer::I_memcmp() {
    uint64_t a = Arg(1), b = Arg(2), n = Arg(3);
    for (uint64_t i = 0; i < n; ++i) {
        int diff = static_cast<int>(mem_.Read8(a+i)) - static_cast<int>(mem_.Read8(b+i));
        if (diff) { Ret(static_cast<uint64_t>(static_cast<int32_t>(diff))); return; }
    }
    Ret(0);
}
void WinAPILayer::I_memchr() {
    uint64_t p = Arg(1); uint8_t c = static_cast<uint8_t>(Arg(2)); uint64_t n = Arg(3);
    for (uint64_t i = 0; i < n; ++i) if (mem_.Read8(p+i) == c) { Ret(p+i); return; }
    Ret(0);
}
void WinAPILayer::I_atoi() { Ret(static_cast<int32_t>(std::atoi(mem_.ReadStringA(Arg(1)).c_str()))); }
void WinAPILayer::I_atol() { Ret(static_cast<int32_t>(std::atol(mem_.ReadStringA(Arg(1)).c_str()))); }
...
void WinAPILayer::I_srand() { std::srand(static_cast<unsigned>(Arg(1))); Ret(0); }
void WinAPILayer::I_time()  {
    std::time_t t = std::time(nullptr);
    const uint64_t p = Arg(1); if (p) mem_.Write64(p, static_cast<uint64_t>(t));
...
void WinAPILayer::I_fread() {
    ...
    uint64_t buf = Arg(1); uint64_t sz = Arg(2); uint64_t cnt = Arg(3);
    if (!f) { Ret(0); return; }
...
void WinAPILayer::I_fseek() {
    FILE* f = GetFile(Arg(1));
    if (f) std::fseek(f, static_cast<long>(Arg(2)), static_cast<int>(Arg(3)));
    Ret(0);
}
...
void WinAPILayer::I_feof() { FILE* f = GetFile(Arg(1)); Ret(f ? static_cast<uint64_t>(std::feof(f)) : 1); }
void WinAPILayer::I_fflush() { FILE* f = GetFile(Arg(1)); if(f) std::fflush(f); Ret(0); }
...
void WinAPILayer::I_rewind()   { FILE* f=GetFile(Arg(1)); if(f) std::rewind(f); Ret(0); }
...
void WinAPILayer::I_GetMessageA() {
    ...
    if (pMsg) { mem_.Write32(pMsg + 4, 0x0012); mem_.Write32(pMsg + 8, 0); }
...
void WinAPILayer::I_GetClientRect()    { const uint64_t p=Arg(2); if(p){mem_.Write32(p,0);mem_.Write32(p+4,0);mem_.Write32(p+8,1920);mem_.Write32(p+12,1080);} Ret(1); }
...
void WinAPILayer::I_D3D11CreateDevice() {
    std::cerr << "[WinAPI] D3D11CreateDevice called — DirectX not yet supported.\n";
    Ret(kE_NOTIMPL);
}
void WinAPILayer::I_D3D11CreateDeviceAndSwapChain() {
    std::cerr << "[WinAPI] D3D11CreateDeviceAndSwapChain called — DirectX not yet supported.\n";
    Ret(kE_NOTIMPL);
}

// ── XInput ────────────────────────────────────────────────────────────────────
void WinAPILayer::I_XInputGetState()        { Ret(1167); } // ERROR_DEVICE_NOT_CONNECTED
