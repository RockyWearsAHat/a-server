...
public:
    ...
    // ── HANDLE table ────────────────────────────────────────────────────
    enum HandleType { HT_INVALID=0, HT_FILE=1, HT_MUTEX=2,
                      HT_EVENT=3, HT_THREAD=4, HT_PROCESS=5 };
    struct HandleEntry {
...
private:
    ...
    struct HeapBlock { uint64_t addr; uint64_t size; bool free; };
    std::vector<HeapBlock> heapBlocks_;

    // ── File handles ─────────────────────────────────────────────────────
    std::vector<FILE*> fileSlots_; // index = internal slot id, stored in HandleEntry.data
    ...
    uint64_t processHeapHandle_ = 0x10001; // fake HANDLE value for the default heap
