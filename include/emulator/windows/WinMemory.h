...
public:
  ...
  uint64_t LoadPEFromData(const std::vector<uint8_t> &data,
                          const std::string &name, uint64_t preferredBase = 0);
  ...
  using ImportResolver =
      std::function<uint64_t(const std::string &dll, const std::string &func)>;
