...
public:
  ...
  bool Initialize(uint64_t imageBase, uint64_t heapBase, uint64_t stackBase,
                  uint64_t stackSize, const std::string &imagePath);
...
private:
  ...
  uint64_t WriteProcessParameters(uint64_t imageBase, uint64_t envBlockAddr);
