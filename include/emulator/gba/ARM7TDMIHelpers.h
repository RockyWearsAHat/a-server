...
namespace AIO::Emulator::GBA::ARM7TDMIHelpers {
     ...
     */
    inline uint32_t GetCPUMode(uint32_t cpsr) {
        return cpsr & CPSR::MODE_MASK;
    }
    ...
    inline bool ConditionSatisfied(uint32_t condition, uint32_t cpsr) {
        ...
        switch (condition) {
            case Condition::EQ: return z;                   // Z set
            case Condition::NE: return !z;                  // Z clear
            case Condition::CS: return c;                   // C set (CS/HS)
            case Condition::CC: return !c;                  // C clear (CC/LO)
            case Condition::MI: return n;                   // N set
            case Condition::PL: return !n;                  // N clear
            case Condition::VS: return v;                   // V set
            case Condition::VC: return !v;                  // V clear
            case Condition::HI: return c && !z;             // C set and Z clear
            case Condition::LS: return !c || z;             // C clear or Z set
            case Condition::GE: return n == v;              // N == V
            case Condition::LT: return n != v;              // N != V
            case Condition::GT: return !z && (n == v);      // Z clear and N == V
            case Condition::LE: return z || (n != v);       // Z set or N != V
            case Condition::AL: return true;                // Always
            case Condition::NV: return false;               // Never (reserved)
     ...
     */
    inline uint32_t RotateRight(uint32_t value, uint32_t amount, uint32_t& cpsr, bool updateCarry = false) {
     ...
     * @return The shifted value
     */
    inline uint32_t BarrelShift(uint32_t value, uint32_t shiftType, uint32_t shiftAmount, 
                                uint32_t& cpsr, bool updateCarry = false) {
        if (shiftAmount == 0 && shiftType != Shift::RRX) {
            return value;
        }
     ...
     */
    inline bool DetectSubBorrow(uint32_t a, uint32_t b) {
        return a < b;
    }
