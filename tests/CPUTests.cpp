...
TEST_F(CPUTest, DataProcessing_Compare) {
    ...
    // -10 = 0xFFFFFFF6
    // CMN R0, #0 (Wait, CMN adds. We need immediate -10? No, immediate is unsigned 8-bit rotated)
    // Let's use register for -10.
    // MOV R1, #-10
...
TEST_F(CPUTest, Thumb_Stack) {
    ...
    EXPECT_EQ(memory.Read32(0x03007F00 - 4), 0xCAFEBABE); // Higher reg at higher addr
